...
// The French engine over arbitrary Unicode — the first Romance / accented-Latin language. Stresses
// the accented alphabet (é/à/ç/ù), the ligatures œ/æ, French elision apostrophes (`l'`, `j'`), and
// the precomposed-NFC lookup path under `Normalization::None`. Asserts every token span is in-bounds,
...
fn engine() -> Option<&'static NativeEngine> {
    ...
    ENGINE
        .get_or_init(|| {
            let disambig = Path::new(RES).join("fr/disambig.rkyv");
            NativeEngine::from_paths(
                &rlt_lang::FR,
                &Path::new(RES).join("segment.srx"),
                &Path::new(RES).join("fr/tagger.rkyv"),
