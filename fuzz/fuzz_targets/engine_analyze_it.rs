...
// The Italian engine over arbitrary Unicode — the first FSA5-format dict (read by the FSA5 sibling
// reader, ISO-8859-15 decoded). Latin + precomposed accents. Asserts every token span is in-bounds,
...
fn engine() -> Option<&'static NativeEngine> {
    ...
    ENGINE
        .get_or_init(|| {
            let disambig = Path::new(RES).join("it/disambig.rkyv");
            NativeEngine::from_paths(
                &rlt_lang::IT,
                &Path::new(RES).join("segment.srx"),
                &Path::new(RES).join("it/tagger.rkyv"),
